...
const AddBottle = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
        const response = await publicRequest.post('/item/create', {title, desc, img, medium, large})
  ...
  return (
      <form className="grid gap-2 my-3 w-11/12">
        ...
        <label className="block">
          <img src={img || null} alt="image upload" className="w-12 h-12 object-cover"/>
          <input
            type="file"
            name="img"
        ...
        {success && <Success message="success adding product, please refresh page!"/>}
