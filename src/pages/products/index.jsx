...
export default function Products() {
  ...
  return (
    <Layout>
      <main className="sm:pt-20 pt-14 bg-gradient-custom">
        <Container>
          <div className="flex w-full justify-between text-amber-900 pb-2 md:pb-0 items-center border-b-2 border-amber-900 md:border-none">
            <Title title="bottle and cups" styledCustom="border-b-2 border-amber-800 pb-2"/>
            {/* cart */}
            <Cart />
            {/* end cart */}
          </div>
          ...
          <div className="flex justify-center">
            <Title title="coffee packs" styledCustom="text-center text-amber-800 border-b-2 pb-2 border-amber-800"/>
          </div>
          <GridMdFour styledCustom="mt-7">
            {packs.map((obj, i) => (
...
function BottleCups(props) {
  ...
  function checkQuantity(){
    (countLarge || countMedium) > 0 ? setDisableButton(false) : setDisableButton(true)
  }
  useEffect(() => {
    checkQuantity();
  ...
  return (
    <>
      <div className="relative w-full text-amber-900 rounded card px-1 py-2 sm:max-h-full max-h-96">
          <ImageBasic src={props.img || defaultImage} alt={props.title} styledCustom="h-56 object-contain"/>
              ...
              <div className="mt-4 w-full">
                ...
                <div className="flex-col flex">
                  <div className="flex justify-evenly">
                    <ButtonLink disabled={disableButton} onClick={() => handleWa(obj)}>
                      <BsWhatsapp size={18} />
                      Whatsapp
                    </ButtonLink>
                    <ButtonLink disabled>
  ...
  return (
    <>
      <div className="relative w-full card mb-12 px-1 py-2 sm:max-h-full max-h-96">
        <ImageBasic src={props.img || defaultImage} alt={props.title} styledCustom="h-56 object-contain"/>
        <div className="px-1 block relative leading-relaxed tracking-wide text-amber-900 m-1">
          <p>{props.title || "unknown"}</p>
